...
exports.myprofileInfo = async(req,res,next) =>{
    try{
        ...
        if(exUser){
            ...
            await db.query('INSERT INTO userprofiles(email,name,sex,age,major,smoke,socialmediaid,socialmediaidtype,mbti,hobby,features,ismatched) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',[req.user.email,req.user.name,post.sex,post.age,post.major,post.smoke,post.socialmediaid,post.socialmediaidtype,mbti,hobby,features,'X'])
